...
def test_revoke_then_grant_flips_state() -> None:
    revoke = client.post("/v1/consent/patient-eu-002/revoke", params={"purpose": "research"})
    assert revoke.status_code == 200
    assert revoke.json()["consent"]["research"] is False
    assert client.get("/policy-data").json()["patient-eu-002"]["research"] is False

    grant = client.post("/v1/consent/patient-eu-002/grant", params={"purpose": "research"})
...
def test_unknown_action_rejected() -> None:
    resp = client.post("/v1/consent/patient-eu-001/toggle", params={"purpose": "research"})
    assert resp.status_code == 400


def test_unsupported_purpose_rejected() -> None:
    resp = client.post("/v1/consent/patient-eu-001/revoke", params={"purpose": "billing"})
    assert resp.status_code == 400
